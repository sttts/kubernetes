...
func (e equalMemoryTypes) Equal(a, b *types.Type, currentlyBuildingTypes map[*types.Type]bool) bool {
	if a == b {
		return true
	}
	...
	result := e.equal(a, b, currentlyBuildingTypes)
	...
	case in.Kind == out.Kind:
		...
		case types.Struct:
			...
			for i, inMember := range in.Members {
				...
				if !e.Equal(inMember.Type, outMember.Type, currentlyBuildingTypes) {
					return false
				}
			}
			return true
		case types.Pointer:
			return e.Equal(in.Elem, out.Elem, currentlyBuildingTypes)
		case types.Map:
			return e.Equal(in.Key, out.Key, currentlyBuildingTypes) && e.Equal(in.Elem, out.Elem, currentlyBuildingTypes)
		case types.Slice:
			return e.Equal(in.Elem, out.Elem, currentlyBuildingTypes)
...
func (noEquality) Equal(_, _ *types.Type, _ map[*types.Type]bool) bool { return false }

type TypesEqual interface {
	Equal(a, b *types.Type, currentlyBuildingTypes map[*types.Type]bool) bool
}
...
func (g *genConversion) doStruct(inType, outType *types.Type, sw *generator.SnippetWriter) {
	for _, inMember := range inType.Members {
		...
		// currentlyBuildingTypes holds all the types that have been converted. This is used to avoid
		// recursion for recursive types.
		currentlyBuildingTypes := make(map[*types.Type]bool)
		if g.useUnsafe.Equal(inMemberType, outMemberType, currentlyBuildingTypes) {
			args = args.
