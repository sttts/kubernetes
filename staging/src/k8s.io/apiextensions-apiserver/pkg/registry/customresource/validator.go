...
func (a customResourceValidator) Validate(ctx genericapirequest.Context, obj runtime.Object) field.ErrorList {
	...
	if a.scale != nil {
		...
		specReplicasPath := strings.TrimPrefix(a.scale.SpecReplicasPath, ".") // ignore leading period
		specReplicas, _, err := unstructured.NestedInt64(customResource, strings.Split(specReplicasPath, ".")...)
		if err != nil {
			return field.ErrorList{field.Invalid(field.NewPath(a.scale.SpecReplicasPath), specReplicas, err.Error())}
		}
		if specReplicas < 0 {
			return field.ErrorList{field.Invalid(field.NewPath(a.scale.SpecReplicasPath), specReplicas, "should be a non-negative integer")}
		}
		if specReplicas > math.MaxInt32 {
			return field.ErrorList{field.Invalid(field.NewPath(a.scale.SpecReplicasPath), specReplicas, fmt.Sprintf("should be less than or equal to %v", math.MaxInt32))}
		}

		// validate statusReplicas
		statusReplicasPath := strings.TrimPrefix(a.scale.StatusReplicasPath, ".") // ignore leading period
		statusReplicas, _, err := unstructured.NestedInt64(customResource, strings.Split(statusReplicasPath, ".")...)
		if err != nil {
			return field.ErrorList{field.Invalid(field.NewPath(a.scale.StatusReplicasPath), statusReplicas, err.Error())}
		}
		if statusReplicas < 0 {
			return field.ErrorList{field.Invalid(field.NewPath(a.scale.StatusReplicasPath), statusReplicas, "should be a non-negative integer")}
		}
		if statusReplicas > math.MaxInt32 {
			return field.ErrorList{field.Invalid(field.NewPath(a.scale.StatusReplicasPath), statusReplicas, fmt.Sprintf("should be less than or equal to %v", math.MaxInt32))}
		}

		// validate labelSelector
		labelSelectorPath := strings.TrimPrefix(a.scale.LabelSelectorPath, ".") // ignore leading period
		labelSelector, _, err := unstructured.NestedString(customResource, strings.Split(labelSelectorPath, ".")...)
		if err != nil {
			return field.ErrorList{field.Invalid(field.NewPath(a.scale.LabelSelectorPath), labelSelector, err.Error())}
		}
	}
...
func (a customResourceValidator) ValidateStatusUpdate(ctx genericapirequest.Context, obj, old runtime.Object) field.ErrorList {
	...
	if a.scale != nil {
		...
		// if this lies under .status, validate it
		if splitLabelSelectorPath[0] == "status" {
			labelSelector, _, err := unstructured.NestedString(customResource, splitLabelSelectorPath...)
			if err != nil {
				return field.ErrorList{field.Invalid(field.NewPath(a.scale.LabelSelectorPath), labelSelector, err.Error())}
			}
		}
