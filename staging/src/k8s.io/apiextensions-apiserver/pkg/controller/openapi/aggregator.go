...
// NewAggregationManager constructs a specAggregator from input openAPIVersionedService and
// recorded static OpenAPI spec. The function returns an AggregationManager interface.
func NewAggregationManager(openAPIVersionedService *handler.OpenAPIService, staticSpec *spec.Swagger) (AggregationManager, error) {
...
// openAPISpecInfo is used to store OpenAPI spec
...
func (s *specAggregator) buildOpenAPISpec() (specToReturn *spec.Swagger, err error) {
	...
	for _, specInfo := range specs {
		if specToReturn == nil {
			// TODO (roycaihw): remove after 71223 is merged
