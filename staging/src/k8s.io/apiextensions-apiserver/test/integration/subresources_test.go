...
func TestScaleSubResource(t *testing.T) {
	...
	if gottenScale.Spec.Replicas != 3 {
		t.Fatalf("Scale.Spec.Replicas: expected: %v, got: %v", 5, gottenScale.Spec.Replicas)
	}
...
func TestValidateOnlyStatus(t *testing.T) {
	...
	// validation passes becauses spec is not validated (and also not set)
	...
	err = unstructured.SetNestedField(gottenNoxuInstance.Object, int64(5), "status", "num")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
