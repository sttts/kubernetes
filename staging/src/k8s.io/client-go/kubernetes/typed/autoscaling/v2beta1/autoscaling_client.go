...
func (c *AutoscalingV2beta1Client) ScopedHorizontalPodAutoscalers(scope rest.Scope) HorizontalPodAutoscalersGetter {
...
func NewForConfigAndClient(c *rest.Config, h rest.HTTPClient) (*AutoscalingV2beta1Client, error) {
	config := *c
	if err := setConfigDefaults(&config); err != nil {
		return nil, err
	}
...
// NewWithScope creates a new AutoscalingV2beta1Client for the given RESTClient and scope.
func NewWithScope(c rest.Interface, scope rest.Scope) *AutoscalingV2beta1Client {
	return &AutoscalingV2beta1Client{restClient: c, scope: scope}
}
