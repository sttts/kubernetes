...
type Scoper interface {
	ScopedDiscovery(scope rest.Scope) DiscoveryInterface
}
...
func (d *DiscoveryClient) Scope(scope rest.Scope) DiscoveryInterface {
...
func (d *DiscoveryClient) ServerGroups() (apiGroupList *metav1.APIGroupList, err error) {
	...
	err = d.restClient.Get().Scope(d.scope).AbsPath(d.LegacyPrefix).Do(context.TODO()).Into(v)
	...
	err = d.restClient.Get().Scope(d.scope).AbsPath("/apis").Do(context.TODO()).Into(apiGroupList)
	if err != nil && !errors.IsNotFound(err) && !errors.IsForbidden(err) {
		return nil, err
	}
...
func (d *DiscoveryClient) ServerResourcesForGroupVersion(groupVersion string) (resources *metav1.APIResourceList, err error) {
	...
	err = d.restClient.Get().Scope(d.scope).AbsPath(url.String()).Do(context.TODO()).Into(resources)
	if err != nil {
...
func (d *DiscoveryClient) ServerVersion() (*version.Info, error) {
	body, err := d.restClient.Get().Scope(d.scope).AbsPath("/version").Do(context.TODO()).Raw()
	if err != nil {
		return nil, err
	}
...
func (d *DiscoveryClient) OpenAPISchema() (*openapi_v2.Document, error) {
	data, err := d.restClient.Get().Scope(d.scope).AbsPath("/openapi/v2").SetHeader("Accept", mimePb).Do(context.TODO()).Raw()
	if err != nil {
		if errors.IsForbidden(err) || errors.IsNotFound(err) || errors.IsNotAcceptable(err) {
			// single endpoint not found/registered in old server, try to fetch old endpoint
			// TODO: remove this when kubectl/client-go don't work with 1.9 server
			data, err = d.restClient.Get().Scope(d.scope).AbsPath("/swagger-2.0.0.pb-v1").Do(context.TODO()).Raw()
			if err != nil {
				return nil, err
			}
		} else {
...
func NewDiscoveryClientForConfigAndClient(c *restclient.Config, httpClient rest.HTTPClient) (*DiscoveryClient, error) {
	config := *c
	if err := setDiscoveryDefaults(&config); err != nil {
		return nil, err
	}
