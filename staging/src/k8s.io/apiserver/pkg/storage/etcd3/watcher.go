...
func (w *watcher) Watch(ctx context.Context, key string, rev int64, recursive bool, scope storage.Scope, progressNotify bool, pred storage.SelectionPredicate) (watch.Interface, error) {
	if recursive && !strings.HasSuffix(key, "/") {
		key += "/"
	}
	wc := w.createWatchChan(ctx, key, rev, recursive, scope, progressNotify, pred)
	go wc.run()
...
func (w *watcher) createWatchChan(ctx context.Context, key string, rev int64, recursive bool, scope storage.Scope, progressNotify bool, pred storage.SelectionPredicate) *watchChan {
	wc := &watchChan{
		watcher:           w,
		key:               key,
		initialRev:        rev,
...
func (wc *watchChan) prepareObjs(e *event) (curObj runtime.Object, oldObj runtime.Object, err error) {
	...
	if !e.isDeleted {
		...
		scope := wc.scope.FromPrefixAndKey(wc.key, e.key)
		if err := scope.PostDecode(curObj); err != nil {
			return nil, nil, err
		}
	}
	...
	if len(e.prevValue) > 0 && (e.isDeleted || !wc.acceptAll()) {
		...
		scope := wc.scope.FromPrefixAndKey(wc.key, e.key)
		if err := scope.PostDecode(oldObj); err != nil {
			return nil, nil, err
		}
	}
