...
func (s *store) conditionalDelete(
	...
	getCurrentState := s.getCurrentState(ctx, key, v, false, clusterName, shardName)
...
func (s *store) GuaranteedUpdate(
	...
	getCurrentState := s.getCurrentState(ctx, preparedKey, v, ignoreNotFound, clusterName, shardName)
...
func (s *store) getCurrentState(ctx context.Context, key string, v reflect.Value, ignoreNotFound bool, clusterName logicalcluster.Name, shardName endpointsrequest.Shard) func() (*objState, error) {
...
func (s *store) getState(ctx context.Context, getResp *clientv3.GetResponse, key string, v reflect.Value, ignoreNotFound bool, clusterName logicalcluster.Name, shardName endpointsrequest.Shard) (*objState, error) {
	state := &objState{
		meta: &storage.ResponseMeta{},
	}
