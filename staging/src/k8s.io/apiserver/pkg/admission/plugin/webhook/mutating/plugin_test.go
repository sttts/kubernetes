...
func TestAdmit(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.Name, func(t *testing.T) {
			...
			if tt.ExpectReinvoke != reinvocationCtx.ShouldReinvoke() {
				t.Errorf("expected reinvocationContext.ShouldReinvoke()=%t, but got %t", tt.ExpectReinvoke, reinvocationCtx.ShouldReinvoke())
			}
			for webhook, expectReinvoke := range tt.ExpectReinvokeWebhooks {
				shouldReinvoke := reinvocationCtx.ShouldInvokeWebhook(webhook)
