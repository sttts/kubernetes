...
func (s *mustMatchPatterns) Validate(pod *api.Pod) field.ErrorList {
	...
	if len(pod.Spec.SecurityContext.Sysctls) > 0 {
		if s.patterns == nil {
			allErrs = append(allErrs, field.Invalid(field.NewPath("securityContext").Child("sysctls"), pod.Spec.SecurityContext.Sysctls, "Sysctls are not allowed to be used"))
		} else {
			for i, sysctl := range pod.Spec.SecurityContext.Sysctls {
				allErrs = append(allErrs, s.ValidateSysctl(sysctl.Name, field.NewPath("securityContext").Child("sysctls").Index(i))...)
			}
		}
	}
