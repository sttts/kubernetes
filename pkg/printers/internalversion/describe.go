...
func (d *SecurityContextConstraintsDescriber) Describe(namespace, name string, s printers.DescriberSettings) (string, error) {
	scc, err := d.Core().SecurityContextConstraints().Get(name, metav1.GetOptions{})
	if err != nil {
		return "", err
	}
...
func describePod(pod *api.Pod, events *api.EventList) (string, error) {
	return tabbedString(func(out io.Writer) error {
		...
		if len(pod.Annotations["openshift.io/scc"]) > 0 {
			w.Write(LEVEL_0, "Security Policy:\t%s\n", pod.Annotations["openshift.io/scc"])
		}
