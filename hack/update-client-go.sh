...
if [ "${USE_TEMP_DIR:-1}" = 1 ]; then
    TEMP_STAGING_GOPATH=$(mktemp -d -t verify-staging-client-go)
...
if [ "${SYNC}" != true ]; then
    echo "Cloning k8s.io/apimachinery master from github"
    rm -rf "${TEMP_STAGING_GOPATH}/src/k8s.io/apimachinery"
    go get -d k8s.io/apimachinery/...
fi

echo "Running staging/copy.sh"
staging/copy.sh 2>&1 | sed 's/^/  /'
