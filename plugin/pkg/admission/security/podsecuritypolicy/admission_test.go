...
func TestAdmitSysctls(t *testing.T) {
	podWithSysctls := func(sysctls []string) *kapi.Pod {
		...
		for _, s := range sysctls {
			pod.Spec.SecurityContext.Sysctls = append(pod.Spec.SecurityContext.Sysctls, kapi.Sysctl{s, "dummy"})
		}
		return pod
	}
	...
	for k, v := range tests {
		input := make([]kapi.Sysctl, len(v.pod.Spec.SecurityContext.Sysctls))
		for i := range v.pod.Spec.SecurityContext.Sysctls {
			s := v.pod.Spec.SecurityContext.Sysctls[i]
			input[i] = kapi.Sysctl{s.Name, s.Value}
		}
		testPSPAdmit(k, v.psps, v.pod, v.shouldPass, v.expectedPSP, t)

		if v.shouldPass {
			ok := true
			if len(v.pod.Spec.SecurityContext.Sysctls) != len(input) {
				ok = false
			} else {
				for i := range v.pod.Spec.SecurityContext.Sysctls {
					if v.pod.Spec.SecurityContext.Sysctls[i].Name != input[i].Name {
						ok = false
						break
					}
				}
			...
			if !ok {
				t.Errorf("%s: expected=%v, got=%v", k, input, v.pod.Spec.SecurityContext.Sysctls)
			}
		}
	}
}
