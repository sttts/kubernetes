...
import (
	...
	"k8s.io/kubernetes/cmd/sample-generic-controlplane/app"
...
func StartTestServer(t Logger, instanceOptions *TestServerInstanceOptions, customFlags []string, storageConfig *storagebackend.Config) (result TestServer, err error) {
	...
	s := controlplaneapiserver.NewOptions()
	...
	completedOptions, err := s.Complete(nil, nil)
	if err != nil {
	...
	t.Logf("Starting sample-generic-controlplane on port %d...", s.SecureServing.BindPort)
