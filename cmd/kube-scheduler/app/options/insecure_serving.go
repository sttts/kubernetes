...
// CombinedInsecureServingOptions sets up up to two insecure listener for healthz and metrics. The flags
...
func updateInsecureServingOptionsFromAddress(is *controlleroptions.InsecureServingOptions, addr string) error {
	...
	if err != nil {
		return fmt.Errorf("invalid non-numerical healthz port %q", port)
	}
