...
func (rs *SubdirRepoStore) GetRepo(id string) (*repository.Repository, error) {
	...
	subdirTreeEncoded := rs.repo.Storer.NewEncodedObject()
	if err := subdirTree.Encode(subdirTreeEncoded); err != nil {
		return nil, fmt.Errorf("failed to encode subtree %s of HEAD: %v", id, err)
	}

	subdirTreeHash, err := rs.repo.Storer.SetEncodedObject(subdirTreeEncoded)
	if err != nil {
		return nil, fmt.Errorf("failed to store subtree %s of HEAD: %v", id, err)
	}
