...
func (s *KubeControllerManagerOptions) AddFlags(fs *pflag.FlagSet, allControllers []string, disabledByDefaultControllers []string) {
	...
	fs.DurationVar(&s.Generic.ComponentConfig.ServiceSyncPeriod.Duration, "service-sync-period", s.Generic.ComponentConfig.ServiceSyncPeriod.Duration, "The period for syncing services with their external load balancers")
